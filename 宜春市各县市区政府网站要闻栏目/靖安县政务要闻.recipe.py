...
class jinganyaowen(BasicNewsRecipe):

    title = '靖安县政务要闻'
    description = '靖安县政府网站政务要闻专栏'
#   cover_url = 'http://akamaicovers.oreilly.com/images/0636920024972/lrg.jpg'


    url_prefix = 'http://www.jxjaxzf.gov.cn'
    url_prefix2 = 'http://www.jxjaxzf.gov.cn/Category_133/Index.aspx'
    no_stylesheets = True
    keep_only_tags = [{ 'class': 'articleCon' }]
...
#   oldest_article = 1  #下载的最旧的文章是几天前的。默认是7天，单位是天。如果文章有日期，这个参数起作用。但是这个日期暂时不知道怎么认定，这个参数在宜春政府网的政务要闻不起作用
    ...
    def parse_index(self):
        urlist = [self.url_prefix2]
        """
        下面的for循环用于拼接多个页面的url，并添加到urlist
        翻页后第2页的链接形式为“http://www.jxjaxzf.gov.cn/Category_133/Index_2.aspx”
        政府网站更新较慢，需根据抓取日期范围决定range范围，翻页少了缺内容，翻多了影响抓取效率
        """
        for nu in range(1,5):
            urlist.append(self.url_prefix + r'/Category_133/Index_' + str(nu) + r'.aspx')
            ...
            for li in ul.findAll('li'):
                ...
                try:
                    ...
                    if days_betwen <= 30 : #限定抓取几天内的新闻，当天的则为days_betwen == 0
                        arti.append(str(li))  # 注意要转换为字符串，beautifusoup不接受列表和其他类型的数据
                except:
                    pass
